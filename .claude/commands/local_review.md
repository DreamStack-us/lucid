...
<!-- ccw-template-name: local_review -->
...
3. **Set up the remote and worktree**:
   ...
   - If not, add it: `git remote add USERNAME git@github.com:USERNAME/loother`
   - Fetch from the remote: `git fetch USERNAME`
   - Create worktree: `git worktree add -b BRANCHNAME ~/wt/loother/SHORT_NAME USERNAME/BRANCHNAME`

4. **Configure the worktree**:
   - Copy Claude settings: `cp .claude/settings.local.json WORKTREE/.claude/`
   - Run setup: `npm install`
   - Initialize thoughts (if using): `cd WORKTREE && npx humanlayer thoughts init --directory loother`
...
- Create worktree at `~/wt/loother/add-new-api` (or ticket number if found)
...
- **Setup command**: Adjust `npm install` to match your project (npm install, poetry install, etc.)
- **Worktree location**: Change `~/wt/loother` if you prefer a different location
