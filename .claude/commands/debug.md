...
**PGMQ Queue Debugging** (Loother-specific):
```bash
# Check queue depth and health
psql $DATABASE_URL -c "SELECT * FROM pgmq_metrics('gear_analysis_queue');"

# View active jobs
psql $DATABASE_URL -c "SELECT msg_id, read_ct, enqueued_at, vt, message->>'gear_item_id' FROM pgmq.q_gear_analysis_queue ORDER BY enqueued_at DESC LIMIT 10;"

# View archived (failed) jobs
psql $DATABASE_URL -c "SELECT msg_id, read_ct, archived_at, message->>'gear_item_id' FROM pgmq.a_gear_analysis_queue ORDER BY archived_at DESC LIMIT 10;"

# Count jobs by status
psql $DATABASE_URL -c "SELECT (SELECT COUNT(*) FROM pgmq.q_gear_analysis_queue) as active, (SELECT COUNT(*) FROM pgmq.a_gear_analysis_queue) as archived;"

# Check gear_analysis_results status
psql $DATABASE_URL -c "SELECT gear_item_id, status, progress, error_message FROM gear_analysis_results ORDER BY created_at DESC LIMIT 10;"
```

**Fly.io Worker Debugging**:
```bash
# Check worker status
fly status --app loother-dev

# View real-time logs
fly logs --app loother-dev

# Check health endpoint
curl https://loother-dev.fly.dev/health

# View worker metrics
curl https://loother-dev.fly.dev/metrics

# SSH into worker for debugging
fly ssh console --app loother-dev

# Check worker machine list
fly machines list --app loother-dev

# View recent deployments
fly releases --app loother-dev
```

Remember: This command helps you investigate without burning the primary window's context. Perfect for when you hit an issue during manual testing and need to dig into logs, database, git state, or Fly.io worker status.
